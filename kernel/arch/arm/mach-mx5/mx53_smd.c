...
#define MX53_MIPAD_DISP0_CONTRAST	(0*32 + 1)	/* GPIO1_1 */
...
#define MX53_MIPAD_HDMI_PWR_EN		(1*32 + 4)	/* GPIO2_4 */
...
#define MX53_MIPAD_USB_HUB_POWER	(1*32 + 12)	/* GPIO2_12 */
#define MX53_SMD_GPS_RESET_B		(1*32 + 12)	/* GPIO2_12 */
#define MX53_MIPAD_LCD_PWR_EN       (1*32 + 13) /* GPIO2_13 */
#define MX53_SMD_WAKEUP_ZIGBEE		(1*32 + 13)	/* GPIO2_13 */
#define MX53_SMD_KEY_VOL_DOWN		(1*32 + 14)	/* GPIO2_14 */
#define MX53_MIPAD_TP_RESET			(1*32 + 14)	/* GPIO2_14 */
...
#define MX53_MDK_KEY_BACK			(2*32 + 11)	/*GPIO_3_11 */
...
#define MX53_MIPAD_USB_HUB_RESET_B	(2*32 + 14) /*GPIO_3_14 */
...
#define MX53_MIPAD_TP_INT			(2*32 + 22)	/* GPIO_3_22 */
#define MX53_MDK_KEY_HOME			(2*32 + 26)	/* GPIO_3_26 */
...
#define MX53_MDK_HDMI_PWR_EN		(3*32 + 4)	/* GPIO_4_4 */
#define MX53_SMD_HDMI_CEC_D			(3*32 + 4)	/* GPIO_4_4 */

#define MX53_MIPAD_GPIO_KEY_OUTPUT	(3*32 + 10) /* GPIO_4_10 */
#define MX53_SMD_MODEM_DISABLE_B	(3*32 + 10)	/* GPIO_4_10 */
#define MX53_MIPAD_KEY_VOL_DOWN		(3*32 + 11) /* GPIO_4_11 */
#define MX53_SMD_SD1_WP				(3*32 + 11)	/* GPIO_4_11 */
#define MX53_SMD_DCDC5V_BB_EN		(3*32 + 14)	/* GPIO_4_14 */
#define MX53_MIPAD_KEY_VOL_UP		(3*32 + 15) /* GPIO_4_15 */
#define MX53_SMD_WLAN_HOST_WAKE		(3*32 + 15)	/* GPIO_4_15 */

#define MX53_MIPAD_HDMI_RESET_B		(4*32 + 0)	/* GPIO_5_0 */
...
#define MX53_MIPAD_HDMI_INT			(5*32 + 12) /* GPIO_6_12 */
#define MX53_SMD_HDMI_INT			(5*32 + 12)	/* GPIO_6_12 */
#define MX53_SMD_LCD_PWR_EN			(5*32 + 13)	/* GPIO_6_13 */
#define MX53_MIPAD_ACCL_INT1_IN		(5*32 + 15)	/* GPIO_6_15 */
...
#define MX53_MIPAD_WIFI_USB_PWR_EN	(6*32 + 2)	/* GPIO_7_2 */
#define MX53_SMD_CABC_EN0			(6*32 + 2)	/* GPIO_7_2 */
#define MX53_MIPAD_VDD_5V_EN		(6*32 + 3)	/* GPIO_7_3 */
...
#define MX53_MDK_USB_OTG_PWR_EN		(6*32 + 8)	/* GPIO_7_8 */
#define MX53_SMD_USB_OTG_PWR_EN		(6*32 + 8)	/* GPIO_7_8 */
#define MX53_MDK_KEY_MENU			(6*32 + 9)	/* GPIO_7_9 */
...
static iomux_v3_cfg_t mx53_smd_pads[] = {
	...
	/* MODEM_DISABLE_B */
	/* MX53_MIPAD_GPIO_KEY_OUTPUT */
	MX53_PAD_KEY_COL2__GPIO4_10,
	/* SD1_WP */
	/* MX53_MIPAD_KEY_VOL_DOWN */
...
static struct fb_videomode video_modes[] = {
	...
	0,},
	{
	/* HSD100IFW1-A00 1024x600 LVDS panel */
	"SXGA", 60, 1024, 600, 19000, 220, 40, 21, 7, 60, 10,
	0,
	FB_VMODE_NONINTERLACED,
	0,},
	{
	/* LP097X02 1024x768 LVDS panel */
	"LP097X02", 60, 1024, 768, 10000, 480, 260, 16, 6, 240, 10,
	0,
	FB_VMODE_NONINTERLACED,
	0,},
	{
	/* 1920x1080P60 HDMI */
	"1920x1080P60", 60, 1920, 1080, 6734, 148, 4, 88, 36, 44, 5,
	FB_SYNC_HOR_HIGH_ACT | FB_SYNC_VERT_HIGH_ACT | FB_SYNC_EXT,
	FB_VMODE_NONINTERLACED,
	0,},
	{
	/* 1280x720P60 HDMI */
	"1280x720P60", 60, 1280, 720, 13515, 220, 110, 20, 5, 40, 5,
	FB_SYNC_HOR_HIGH_ACT | FB_SYNC_VERT_HIGH_ACT | FB_SYNC_EXT,
	FB_VMODE_NONINTERLACED,
	0,},
	{
...
static struct mxc_fb_platform_data fb_data[] = {
	...
	},
	{
	 ...
	 .mode_str = "1920x1080P60",
	 .mode = video_modes,
	 .num_modes = ARRAY_SIZE(video_modes),
	},
	{
	 .interface_pix_fmt = IPU_PIX_FMT_RGB24,
	 .mode_str = "1280x720P60",
	 .mode = video_modes,
	 .num_modes = ARRAY_SIZE(video_modes),
	},
	{
	 .interface_pix_fmt = IPU_PIX_FMT_RGB24,
	 .mode_str = "1024x768P60",
	 .mode = video_modes,
	 .num_modes = ARRAY_SIZE(video_modes),
	},
	{
...
static int __init mxc_init_fb(void)
{
	...
	if (primary_di) {
...
#ifdef CONFIG_MACH_MX53_MIPAD
		mxc_register_device(&mxc_fb_devices[1], &fb_data[2]);
#elif defined(CONFIG_MACH_MX53_MDK)
		mxc_register_device(&mxc_fb_devices[1], &fb_data[6]);
#else
		...
		/* DI0 -> DC channel: */
#ifdef CONFIG_MACH_MX53_MIPAD
		mxc_register_device(&mxc_fb_devices[0], &fb_data[4]);
#else
...
{
	gpio_set_value(MX53_MIPAD_HDMI_RESET_B, 0);
	msleep(10);
	gpio_set_value(MX53_MIPAD_HDMI_RESET_B, 1);
	msleep(10);
}

static struct mxc_lcd_platform_data hdmi_data = {
	.reset = hdmi_reset,
};
...
static int get_touchscreen_irq_status(void)
{
	return gpio_get_value(MX53_MIPAD_TP_INT);
}

static void set_touchscreen_reset_val(int on)
{
	gpio_set_value(MX53_MIPAD_TP_RESET, on);
}

static struct mxc_touchscreen_platform_data mxc_touchscreen_data = {
	.init_gpio = NULL,
	.deinit_gpio = NULL,
	.set_reset_val = set_touchscreen_reset_val,
	.get_irq_status = get_touchscreen_irq_status,
	.irq = gpio_to_irq(MX53_MIPAD_TP_INT),
};
...
static struct i2c_board_info mxc_i2c1_board_info[] __initdata = {
	...
	.type = "ssd2533_ts",
	.addr = 0x48,
	.platform_data = (void *)&mxc_touchscreen_data,
	},
};
...
static struct i2c_board_info mxc_i2c2_board_info[] __initdata = {
	...
	.type = "it6610",
	.addr = 0x4d,
	.irq = gpio_to_irq(MX53_MIPAD_HDMI_INT),
...
volatile int tp_connected = 0;

void tp_notify(int connected)
{
	tp_connected = connected;
}
EXPORT_SYMBOL(tp_notify);

static void mx53_gpio_usbotg_driver_vbus(bool on)
{
#ifndef CONFIG_MACH_MX53_MDK
	if (on)
...
#ifdef CONFIG_MACH_MX53_MDK
	/* FEC PWR enable*/
	gpio_request(MX53_SMD_FEC_PWR_EN, "fec-pwr-en");
	gpio_direction_output(MX53_SMD_FEC_PWR_EN, 1);

	/* IT6610 hdmi */
	gpio_request(MX53_MDK_HDMI_PWR_EN, "hdmi-pwr-en");
	gpio_direction_output(MX53_MDK_HDMI_PWR_EN, 1);
#endif
...
#ifdef CONFIG_MACH_MX53_MDK
	/* USB host 3V3 enable*/
	gpio_request(MX53_MDK_USB_OTG_PWR_EN, "usb-otg-pwr-en");
	gpio_direction_output(MX53_MDK_USB_OTG_PWR_EN, 1);
#else
	/* USB PWR enable */
...
#ifdef CONFIG_MACH_MX53_MIPAD
	gpio_request(MX53_MIPAD_LCD_PWR_EN, "lcd-pwr-en");
	gpio_direction_output(MX53_MIPAD_LCD_PWR_EN, 1);
#else
...
#ifdef CONFIG_MACH_MX53_MIPAD
	/* lcd power */
	gpio_request(MX53_MIPAD_DISP0_CONTRAST, "disp0-contrast");
	gpio_direction_output(MX53_MIPAD_DISP0_CONTRAST, 1);

	/* wifi usb power */
	gpio_request(MX53_MIPAD_VDD_5V_EN, "vdd-5v-en");
	gpio_direction_output(MX53_MIPAD_VDD_5V_EN, 1);

	gpio_request(MX53_MIPAD_WIFI_USB_PWR_EN, "wifi-usb-pwr-en");
	gpio_direction_output(MX53_MIPAD_WIFI_USB_PWR_EN, 1);

	/* usb hub power */
	gpio_request(MX53_MIPAD_USB_HUB_POWER, "usb-hub-power");
	gpio_direction_output(MX53_MIPAD_USB_HUB_POWER, 1);

	gpio_request(MX53_MIPAD_USB_HUB_RESET_B, "usb-hub-reset-b");
	gpio_direction_output(MX53_MIPAD_USB_HUB_RESET_B, 0);
	msleep(10);
	gpio_direction_output(MX53_MIPAD_USB_HUB_RESET_B, 1);

	gpio_request(MX53_MIPAD_GPIO_KEY_OUTPUT, "gpio-key-output");
	gpio_direction_output(MX53_MIPAD_GPIO_KEY_OUTPUT, 1);

	/* IT6610 HDMI */
	gpio_request(MX53_MIPAD_HDMI_PWR_EN, "hdmi-pwr-en");
	gpio_direction_output(MX53_MIPAD_HDMI_PWR_EN, 1);
	gpio_request(MX53_MIPAD_HDMI_RESET_B, "hdmi-reset-b");
	gpio_direction_output(MX53_MIPAD_HDMI_RESET_B, 0);
	gpio_request(MX53_MIPAD_HDMI_INT, "hdmi-int");
	gpio_direction_input(MX53_MIPAD_HDMI_INT);

	/* TP interrupt reset gpio */
	gpio_request(MX53_MIPAD_TP_INT, "tp-int");
	gpio_direction_input(MX53_MIPAD_TP_INT);
	gpio_request(MX53_MIPAD_TP_RESET, "tp-reset");
	gpio_direction_output(MX53_MIPAD_TP_RESET, 0);
#endif
